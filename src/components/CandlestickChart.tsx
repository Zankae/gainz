...
export default function CandlestickChart({ exercise }: CandlestickChartProps) {
  ...
  const [zoom, setZoom] = useState(52); // Show last N candles
  ...
  const visible = useMemo(() => {
    ...
    return candles.slice(Math.max(0, candles.length - zoom));
  }, [candles, zoom]);

  // Read CSS tokens once per render (var() doesn't work in SVG attributes)
  ...
  }, [theme]); // Recompute on theme change

  const layout = useMemo(() => {
    return candleLayout(visible.length, 320 - CHART.AXIS_W);
  }, [visible.length]);

  const scale = useMemo(() => {
    return priceScale(visible);
  }, [visible]);

  const volMax = useMemo(() => {
    if (visible.length === 0) return 1;
    return Math.max(...visible.map(c => c.volumeKg));
  }, [visible]);
  ...
  const candleBodyY = useCallback((c: Candle) => {
    ...
    return {
      y: scale.y(top),
      h: Math.max(1, scale.y(bottom) - scale.y(top)),
    };
  }, [scale]);
  ...
  if (candles.length === 0) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: 'var(--muted)' }}>
        No data yet for {exercise.name}. Complete a workout to see progress.
      </div>
    );
  }
    ...
    <div style={{ marginTop: 12 }}>
      ...
      {candles.length > 4 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          <input
            type="range"
            min={Math.min(candles.length, 4)}
            max={candles.length}
            value={zoom}
            onChange={(e) => setZoom(Number(e.target.value))}
            style={{ flex: 1, accentColor: 'var(--accent)' }}
          />
          <span style={{ fontSize: 11, color: 'var(--muted)', minWidth: 30, fontFamily: 'var(--font-display)' }}>
            {zoom}
          </span>
        </div>
      )}
      ...
      <div
        ref={scrollRef}
        style={{
          overflowX: layout.scrolls ? 'auto' : 'hidden',
      ...
      >
        <svg
          width={Math.max(320, layout.contentWidth)}
          height={CHART_TOTAL_H}
          viewBox={`0 0 ${Math.max(320, layout.contentWidth)} ${CHART_TOTAL_H}`}
        >
          {/* Gridlines */}
          {scale.lines.map((v, i) => (
            <g key={i}>
              <line
                x1={0}
                x2={Math.max(320, layout.contentWidth) - CHART.AXIS_W}
                y1={scale.y(v)}
                y2={scale.y(v)}
                stroke={colors.grid}
                strokeWidth="0.5"
              />
              <text
                x={Math.max(320, layout.contentWidth) - CHART.AXIS_W + 4}
                y={scale.y(v) + 4}
                fill={colors.muted}
                fontSize="10"
          ...
          {metric === 'weight' && visible.map((c, i) => {
            ...
            const opacity = c.up ? '0.3' : '0.25';
            return (
              <rect
                key={`vol-${c.key}`}
                x={i * layout.slot + (layout.slot - layout.candleW) / 2}
          ...
          {visible.map((c, i) => {
            ...
            const color = c.up ? colors.gain : colors.loss;
            const cx = i * layout.slot + layout.slot / 2;
          ...
          {/* Right axis line */}
          <line
            x1={Math.max(320, layout.contentWidth) - CHART.AXIS_W}
            x2={Math.max(320, layout.contentWidth) - CHART.AXIS_W}
            y1={0}
            y2={CHART_TOTAL_H}
            stroke={colors.grid}
      ...
      {selectedCandle && (
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, fontSize: 13 }}>
            ...
            <div style={{ fontFamily: 'var(--font-display)', textAlign: 'right' }}>
              {selectedCandle.o.toFixed(metric === 'weight' ? 2 : 0)}
            </div>
            <div style={{ color: 'var(--muted)' }}>High</div>
            <div style={{ fontFamily: 'var(--font-display)', textAlign: 'right', color: 'var(--gain)' }}>
              {selectedCandle.h.toFixed(metric === 'weight' ? 2 : 0)}
            </div>
            <div style={{ color: 'var(--muted)' }}>Low</div>
            <div style={{ fontFamily: 'var(--font-display)', textAlign: 'right', color: 'var(--loss)' }}>
              {selectedCandle.l.toFixed(metric === 'weight' ? 2 : 0)}
            </div>
            <div style={{ color: 'var(--muted)' }}>Close</div>
            <div style={{ fontFamily: 'var(--font-display)', textAlign: 'right' }}>
              {selectedCandle.c.toFixed(metric === 'weight' ? 2 : 0)}
            </div>
            <div style={{ color: 'var(--muted)' }}>Volume</div>
            <div style={{ fontFamily: 'var(--font-display)', textAlign: 'right' }}>
              {Math.round(selectedCandle.volumeKg)} kg
            </div>
